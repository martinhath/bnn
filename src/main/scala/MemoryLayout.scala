...
object MemoryLayout {
  ...
  def getStreams(parameters: LayerParameters,
    ...
    val unshifted_PUstreams = Range(0, parameters.NumberOfPUs).map(
      pui =>
        weights.zipWithIndex
          .filter({ case (row, i) => i % parameters.NumberOfPUs == pui })
          .flatMap(pair => pair._1))

    val shifted_PUstreams = unshifted_PUstreams.zipWithIndex.map({
      case (s, i) => rotate(s, parameters.K * (i % PUsPerMUs))
    })

    val PU_word_streams =
      shifted_PUstreams.map(s => s.grouped(parameters.K).toList)

    val MU_seperated =
      PU_word_streams.grouped(PUsPerMUs).map(_.transpose).toList

    val MU_word_streams = MU_seperated.map(
      _.map(_.reverse.flatMap(_.reverse).mkString).map("b" + _))
    ...
    val biasStream = bias
      ...
      .flatMap(_ ++ List.fill(
        parameters.MatrixWidth / parameters.K - parameters.NumberOfPUs)(0))
      .map("b" + paddedBinary(_, parameters.BiasWidth))
      .toArray
